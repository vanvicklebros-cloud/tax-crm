...
app.get('/api/leads', requireAuth, (req, res) => {
  ...
  if (q) { where.push('(name LIKE :q OR email LIKE :q OR address LIKE :q OR county LIKE :q OR town LIKE :q)'); params.q = `%${q}%`; }
...
app.get('/api/export', requireAuth, (req, res) => {
  ...
  for (const l of leads) {
    rows.push(headers.map(h => { const v = (l[h] || '').toString().replace(/"/g, '""'); return `"${v}"`; }).join(','));
  }
...
app.post('/webhook', (req, res) => {
  ...
  if (webhookSecret && req.query.secret !== webhookSecret) return res.status(401).json({ error: 'Unauthorized' });
  try {
    const body = req.body;
    ...
    db.prepare(`INSERT INTO leads (received_at, name, email, phone, address, town, county, property_type, assessed_value, message, source, status, raw_data) VALUES (datetime('now'), :name, :email, :phone, :address, :town, :county, :property_type, :assessed_value, :message, :source, 'New', :raw_data)`).run({
    ...
    console.log(`[webhook] New lead from ${source}`);
